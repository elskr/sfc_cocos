#F_i =  semble aider, mais du coup pas mal de variables ne convergent pas. Soit remettre F_i à sa valeur d'origine et tout converge mais il faut régler le souci des valeurs négayives, soit laisser comme ça et essayer de faire converger, soit en faisant tourner le modèle plus longtemps (probable), soit en modifiant des valeurs, soit en modifiant des équations
#update 05/02/2019 : ça semble mieux fonctionner comme ça, le modèle converge mais vers des valeurs un peu bizarres ; pour y remédier, j'essaye d'obtenir de bonnes valeur en t=1 ; modifier le F_b1 et F_b2 comme pour F_i (cf note précédente) pour avoir un FU_b1 et FU_b2 poitifs en période t=1
#update 09/02/2019 : problème des profits distribués réglé, mais taux d'utilisation à 6.90, alors qu'il ne devrait pas l'être ; éventuellement le passer en ratio nominal
#update 18/02/2019 : inventaires devient négatif en périodes ultérieures car production < ventes -> pas possible ; ils peuvent diminuer mais pas devenir négatif, régler ça (soit en augmentant la taille initiale des inventaires et en vérifiant qu'ils s'arrêtent de décroire à un moment donné, soit en faisant en sorte que les inventaires ne décroient pas ou moins) -> réglé en augmentant 2 paramètres d'ajustement des inventaires

#update 18/02/2019 : salaire peut être un peu trop élevé (à vérifier en fonction des valeurs de long terme), évengtuellement baisser le salaire unitaire, ce qui semble poser problème
#update 11/03/2019 : paramètres d'arbitrage de portefeuille divisés par 10 pour éviter des valeurs négatives mais éventuellement à remodifier comme avant (j'ai l'impression que ça a induit encore plus de négatifs, mais peut être qu'il faut encore changer d'autres paramètres)
#update 19/03/2019 : prix des BCO2 négatif
#update 31/03/219 : tout semble ok, j'ai ajouté les Vfma pour les firmes afin que la conversion en coco equities puisse avoir un impact sur ce secteurs
#update 10/04/2019 : suppression des investisseurs institutionnels (cf bizarre2.py pour l'ancienne version)
...
def create_kremer_model():
    ...
    model.var('HPM_rd', desc="demande des menages investisseurs en cash")
    model.var('HPM_rh', desc="cash detenu par les menages investisseurs")
    model.var('HPM_rs', desc="cash offert aux menages investisseurs")
    model.var('HPM_wd', desc="demande de cash de la part des menages travailleurs")
    model.var('HPM_wh', desc="cash detenu par les menages travailleurs")
    model.var('HPM_ws', desc="cash offert aux menages travaileurs")
    ...
    model.var('r_a', desc="taux d interet sur les avances")
    model.var('r_b', desc="taux d interet sur les bons du tresor")
    ...
    model.var('Vfma_rgrowth', desc = "taux de croissance de la valeur nette des menages investisseurs")
    ...
    model.param('lambdaC', desc="parametre demande de cash de la part des menages") 
    ...
    model.param('rstar', desc="neutral long term interest rate")
    ...
    model.add('L_wd - L_wd(-1) = NL_wd')
    model.add('BUR_w = (REP_w + r_l(-1) * L_wd(-1)) / YP_w')
    model.add('NPLW = nplw * L_ws(-1)')
    model.add('nplw = nplw0 + nplw1 * BUR_w(-1) - nplw2 * klim(-1)')
    model.add('Vfma_w = V_w + L_ws - HPM_wh')
    model.add('D_wd = Vfma_w')
    model.add('D_ws = D_wd')
    model.add('D_wh = D_ws')
    model.add('HPM_wd = lambdaC * C_w')
    model.add('HPM_ws = HPM_wd')
    model.add('HPM_wh = HPM_ws')
    ...
    model.add('YD_r = YP_r - T_r')
    model.add('CG_r = (pbco1 - pbco1(-1)) * indicKRb1 * BCO1_rh(-1) + (pbco2 - pbco2(-1)) * indicKRb2 * BCO2_rh(-1) + (pe - pe(-1)) * e_rh(-1)')
    model.add('YDHS_r = YD_r + CG_r')
    model.add('V_r - V_r(-1) = YDHS_r - C_r')
    model.add('v_r = V_r / p')
    model.add('c_r = alpha1 * yd_re + alpha2 * v_r(-1)')
    model.add('C_r = p * c_r')
    model.add('yd_r = YD_r / p')
    model.add('yd_re = epsilon * yd_r + (1 - epsilon) * yd_r(-1) * (1 + grpr)')
    model.add('Vfma_r = V_r - HPM_rh - (OFB1_rh + OFB2_rh)')
    ...
    model.add('HPM_rd = lambdaC * C_r')
    model.add('HPM_rs = HPM_rd')
    model.add('HPM_rh = HPM_rs')
    model.add('e_rd = e_rs')
    model.add('B_rs = B_rd')
    model.add('B_rh = B_rs')
    model.add('e_rh = e_rs')
    ...
    model.add('r_q = (L_fd + B_fh + pe * e_s + pbco1 * indicKRb1 * BCO1_fh + pbco2 * indicKRb2 * BCO2_fh) / (K + IN)')
    ...
    model.add('F_f = S - WB + (IN - IN(-1)) - r_l * IN(-1) + r_fTOT + CG_f + FD_fb1 + FD_fb2')    
    model.add('FD_f = psiD * F_f(-1)')
    model.add('FD_rf = (e_rs / e_s) * FD_f')    
    model.add('FD_b2f = (e_b2s / e_s) * FD_f')    
    model.add('FU_f = F_f - FD_f - r_l(-1) * (L_fd(-1) - IN(-1))')
    model.add('L_fd = L_fd(-1) + INV + (IN - IN(-1)) - zeta * FU_f - (e_s - e_s(-1)) * pe')    
    model.add('e_s - e_s(-1) = (1 - psiU) * INV(-1) / pe')    
    model.add('r_K = FD_f / (e_s(-1) * pe(-1))')
    model.add('PE = pe * e_s(-1) / F_f')
    model.add('r_fTOT = r_b(-1) * B_fh(-1) + indicKRb1 * BCO1_fh(-1) + indicKRb2 * BCO2_fh(-1)')
    model.add('CG_f = BCO1_fh(-1) * (pbco1 - pbco1(-1)) * indicKRb1 + BCO2_fh(-1) * (pbco2 - pbco2(-1)) * indicKRb2')
    model.add('V_f - V_f(-1) = (1 - zeta) * FU_f')
    model.add('Vfma_f = V_f - (OFB1_fh + OFB2_fh)')
    model.add('B_fd = Vfma_f - pbco1 * indicKRb1 * BCO1_fs - pbco2 * indicKRb2 * BCO2_fs')
    ...
    model.add('klim = klim0 - klim1 * LEV_w(-1) + klim2 * (CARb1(-1) - CARTb1) - klim3 * BUR_w(-1) - klim4 * nplw')
    ...
    model.add('V_b1 - V_b1(-1) = FU_b1 - NPLW')
    ...
    #model.add('indicKRb1bis = if_true(KR_b1(-1) < KRT)')
    ...
    #model.add('indicKRb2bis = if_true(KR_b2(-1) > KRT)')
    ...
    model.add('Vfma_rgrowth = (Vfma_r - Vfma_r(-1)) / Vfma_r(-1)')
...
for _ in range(300):
    #baseline.solve(iterations=100, threshold=1e-6, debuglist=variables)
    baseline.solve(iterations=300, threshold=1e-6)
    
    #prev_soln = baseline.solutions[-2]
    #soln = baseline.solutions[-1]
    #if is_close(prev_soln, soln, rtol=1e-6):
    #    break

#from pprint import pprint
#pprint(baseline.solutions[-1])
...
for _ in range(30):
    scenar1aa.solve(iterations=200, threshold=1e-6)

scenar1aa.set_values({'KRT1': 0.2})    
scenar1aa.set_values({'nplw0': 0.4})
...
for _ in range(30):
    scenar1ab.solve(iterations=200, threshold=1e-6)

scenar1ab.set_values({'KRT1': 0.2})
scenar1ab.set_values({'nplw0': 0.6})
...
for _ in range(30):
    scenar1ac.solve(iterations=200, threshold=1e-6)
    
scenar1ac.set_values({'KRT1': 0.2})
scenar1ac.set_values({'nplw0': 0.8})
...
for _ in range(30):
    scenar1ba.solve(iterations=200, threshold=1e-6)
    
scenar1ba.set_values({'nplw0': 0.4})
...
for _ in range(30):
    scenar1bb.solve(iterations=200, threshold=1e-6)
    
scenar1bb.set_values({'nplw0': 0.6})
...
for _ in range(30):
    scenar1bc.solve(iterations=200, threshold=1e-6)
    
scenar1bc.set_values({'nplw0': 0.8})
...
g3, = axes.plot(data3, linestyle='-.', color='k', label='+40% shock')
g4, = axes.plot(data4, linestyle=':', color='k', label='+60% shock')
...
caption = '''
    Figure 2 : State of the contingent convertible trigger following an increase in defaulted loans 
...
caption = '''
    Figure 3 : Evolution of the net values of firms, relative to the scenario without
...
g2, = axes.plot(data1, linestyle='--', color='k', label='+20% shock')
g3, = axes.plot(data2, linestyle='-.', color='k', label='+40% shock')
g4, = axes.plot(data3, linestyle=':', color='k', label='+60% shock')
...
caption = '''
    Figure 4 : Evolution of Tobin's Q, relative to the scenario crisis without any activation
    of coco bonds, following an  increase in defaulted loans'''
...
for i in range(5, 100):
    ...
    s1ac = scenar1ac.solutions[i]
    s1bc = scenar1bc.solutions[i]
    data1.append(s1aa['r_q'] / s1ba['r_q'])
    data2.append(s1ab['r_q'] / s1bb['r_q'])
    data3.append(s1ac['r_q'] / s1bc['r_q'])
...
caption = '''
    Figure 5 : Evolution of the investment of firms, relative to the scenario without
    any activation of coco bonds, following an increase in the share of defaulted loans'''
...
caption = '''
    Figure 6 : Evolution of the net value of investing households, relative to the scenario 
...
caption = '''
    Figure 7 : Evolution of the disposable income of investing households, relative to 
...
for i in range(5, 100):
    ...
    s1ac = scenar1ac.solutions[i]
    s1bc = scenar1bc.solutions[i]
    data1.append(s1aa['YD_r'] / s1ba['YD_r'])
    data2.append(s1ab['YD_r'] / s1bb['YD_r'])
    data3.append(s1ac['YD_r'] / s1bc['YD_r'])
...
caption = '''
    Figure 8 : Evolution of the consumption of investing households, relative to the scenario without
    any activation of coco bonds, following an increase in the share of defaulted loans'''
...
caption = '''
    Figure 9 : Evolution of the net value of retails banks, relative to the scenario without
    any activation of coco bonds, following an increase in the share of defaulted loans'''
...
caption = '''
    Figure 10 : Evolution of profits of retails banks, relative to the scenario without
    any activation of coco bonds, following an increase in the share of defaulted loans'''
...
for i in range(5, 100):
    ...
    s1bc = scenar1bc.solutions[i]
    base = baseline.solutions[i]
    data1.append(s1aa['F_b1'] / s1ba['F_b1'])
    data2.append(s1ab['F_b1'] / s1bb['F_b1'])
    data3.append(s1ac['F_b1'] / s1bc['F_b1'])
...
for i in range(5, 45):
    ...
    data5.append(s1ab['Vfma_rgrowth'])
    data6.append(s1bb['Vfma_rgrowth'])
...
for _ in range(40):
    scenar2aa.solve(iterations=200, threshold=1e-6)
    
scenar2aa.set_values({'lambda40': 0.1})
scenar2aa.set_values({'epsilon20': 0.3})
...
for _ in range(40):
    scenar2ab.solve(iterations=200, threshold=1e-6)
    
scenar2ab.set_values({'lambda40': 0.1})
scenar2ab.set_values({'epsilon20': 0.2})
...
#SCENARIO 2Ac : dépréciation du prix des actions d'entreprises avec activation de coco bonds

scenar2ac = create_kremer_model()
scenar2ac.set_values(parameters)
scenar2ac.set_values(exogenous)
scenar2ac.set_values(variables)

for _ in range(40):
    scenar2ac.solve(iterations=200, threshold=1e-6)
    
scenar2ac.set_values({'lambda40': 0.1})
scenar2ac.set_values({'epsilon20': 0.1})
...
for _ in range(40):
    scenar2ba.solve(iterations=200, threshold=1e-6)

scenar2ba.set_values({'lambda40': 0.1})
scenar2ba.set_values({'epsilon20': 0.3})
...
for _ in range(40):
    scenar2bb.solve(iterations=200, threshold=1e-6)

scenar2bb.set_values({'lambda40': 0.1})
scenar2bb.set_values({'epsilon20': 0.2})
...
#SCENARIO 2Bc : dépréciation du prix des actions d'entreprises sans activation de coco bonds

scenar2bc = create_kremer_model()
...
for _ in range(40):
    scenar2bc.solve(iterations=200, threshold=1e-6)

scenar2bc.set_values({'lambda40': 0.1})
scenar2bc.set_values({'epsilon20': 0.1})
...
g2, = axes.plot(data1, linestyle='--', color='k', label='+Baseline')
g3, = axes.plot(data2, linestyle='-.', color='k', label='+35% shock')
g4, = axes.plot(data3, linestyle=':', color='k', label='+45% shock')
...
caption = '''
    Figure 12 : State of the contingent convertible trigger following a decrease in preferences for
...
caption = '''
    Figure 13 : Evolution of the net values of firms, relative to the scenario without any
    activation of coco bonds, following a decrease in preferences for firm equities'''
...
g3, = axes.plot(data1, linestyle='-.', color='k', label='+35% shock')
g4, = axes.plot(data2, linestyle=':', color='k', label='+45% shock')

plt.legend(handles=[g3,g4], prop={'family': 'Times New Roman', 'size':12})

#fig.text(0.1, -.12, caption);

#%%
    
caption = '''
    Figure 14 : Evolution of Tobin's Q, relative to the scenario crisis without any activation
    of coco bonds, following a decrease in the preference for firm equities'''
...
for i in range(20,110):
    ...
    s2ac = scenar2ac.solutions[i]
    s2bc = scenar2bc.solutions[i]
    data1.append(s2aa['r_q'] / s2ba['r_q'])
    data2.append(s2ab['r_q'] / s2bb['r_q'])
...
caption = '''
    Figure 15 : Evolution of the investment of firms, relative to the scenario 
...
caption = '''
    Figure 16 : Evolution of the net values of investing households, relative to the scenario 
    without any activation of coco bonds, following a decrease in preferences for firm equities'''
...
caption = '''
    Figure 17 : Evolution of the disposable income of investing households, relative to the scenario 
    without any activation of coco bonds, following a decrease in preferences for firm equities'''
...
caption = '''
    Figure 18 : Evolution of the consumption of investing households, relative to the scenario 
    without any activation of coco bonds, following a decrease in preferences for firm equities'''
...
caption = '''
    Figure 19 : Evolution of the net value of investment banks, relative to the scenario 
    without any activation of coco bonds, following a decrease in preferences for firm equities'''
...
caption = '''
    Figure 20 : Evolution of the profits of investment banks, relative to the scenario 
    without any activation of coco bonds, following a decrease in preferences for firm equities'''
...
for i in range(20,110):
    ...
    s2ac = scenar2ac.solutions[i]
    s2bc = scenar2bc.solutions[i]
    data1.append(s2aa['F_b2'] / s2ba['F_b2'])
    data2.append(s2ab['F_b2'] / s2bb['F_b2'])
